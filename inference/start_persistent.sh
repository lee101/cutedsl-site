...
setsid nohup env \
    ...
    LOAD_CHRONOS="${LOAD_CHRONOS:-0}" \
    PRELOAD_MODELS=1 \
    ENABLE_NVFP4=0 \
    ZIMAGE_USE_CUTE=1 \
    ZIMAGE_CPU_OFFLOAD="${ZIMAGE_CPU_OFFLOAD:-1}" \
    ZIMAGE_COMPILE_MODE="${ZIMAGE_COMPILE_MODE:-}" \
    ZIMAGE_DEFAULT_STEPS=8 \
    ZIMAGE_FAST_LORA_CPU_OFFLOAD=1 \
