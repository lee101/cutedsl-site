#!/usr/bin/env python3
"""Animation smoothness bench.

Measures requestAnimationFrame cadence and browser long tasks for selected
routes at desktop and mobile viewport sizes. This complements Lighthouse:
Lighthouse is good for page-load metrics, while this catches steady-state
animation jank.

Usage:
    python scripts/homepage_fps_bench.py http://localhost:3000
    python scripts/homepage_fps_bench.py https://cutedsl.cc --seconds 8
    python scripts/homepage_fps_bench.py https://cutedsl.cc --routes / /gallery
"""
import argparse
import statistics
import time
...
def main():
    ...
    parser.add_argument("--routes", nargs="+", default=["/"], help="Routes to measure, e.g. --routes / /gallery")
    ...
    with sync_playwright() as p:
        ...
        for route in args.routes:
            ...
            for name, viewport in VIEWPORTS.items():
                ...
                rows.append(row)
                print(
                    f"  {path} {name}: avg={avg_fps:.1f}fps p95={p95_delta:.1f}ms "
    ...
    lines = [
        ...
        f"timestamp: `{ts}`",
        "",
        "| route | viewport | avg fps | p95 frame ms | dropped frames >34ms | long tasks | long task ms |",
        "|---|---|---:|---:|---:|---:|---:|",
    ]
    for row in rows:
        lines.append(
            f"| `{row['route']}` | {row['viewport']} | {row['avg_fps']:.1f} | {row['p95_frame_ms']:.1f} "
