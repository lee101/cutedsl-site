...
import { ArrowRight, Check, CreditCard, Loader2, Lock, Sparkles, X } from 'lucide-react';
...
export function ArtGenerator({ initialPrompt = '', compact = false, title = 'Make Similar Art', onGenerated }: ArtGeneratorProps) {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/service`, {
        ...
        body: JSON.stringify({ service: 'zimage', prompt: cleanPrompt, width: 1024, height: 1024, num_steps: 8 }),
      });
      const data = await res.json();
      if (!res.ok) {
        if (res.status === 401) setAuthOpen(true);
        if (res.status === 402) setPaymentOpen(true);
